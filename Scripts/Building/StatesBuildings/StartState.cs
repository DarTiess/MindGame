...
{
    public class StartState : State, IDisposable
    {
        ...
        {
            Vector3 rayPoint =
                ray.GetPoint(Vector3.Distance(_scratchObj.transform.position, camera.transform.position));
        ...
        private void OnTrigger(Collider other)
        {
            ...
            if (other.gameObject.TryGetComponent(out MaskSprite mask))
            {
                ...
                if (_isTriggered > Constants.BUILD_TRIGGER)
                {
                    ...
                    _stateMachine.SetState<SecondState>();
                }
            }
        }
