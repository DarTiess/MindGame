...
namespace Infrastructure.UIPanels.Answers
{
    ...
    {
        [Header("Brush")] [SerializeField] private Material brushMaterial;
        [SerializeField] private Vector2 brushScale = new Vector2(0.25f, 0.25f);
        [SerializeField] private int interpolationMaxCount = 4;
        [SerializeField] private int interpolationMinDistance = 8;

        [Header("Mask Texture")] [SerializeField]
        private float maskTextureScale = 1.0f;

        [SerializeField] private float maxMaskTextureSize = 512;

        [Header("Grid")] [SerializeField] [Range(1, 4)]
        ...
        {
            Reload();
            base.RevealProgressChanged += FinicshedDraw;
        }

        private void FinicshedDraw(float progress, bool revealed)
        {
            if (revealed)
            {
        ...
        private static Vector2 ScreenToGraphicLocalPoint(Graphic graphic, Vector2 screenPoint)
        {
            var canvas = graphic.canvas.rootCanvas;
            var camera = canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : canvas.worldCamera;
            var rt = graphic.rectTransform;

            return RectTransformUtility.ScreenPointToLocalPointInRectangle(rt, screenPoint, camera, out var result)
                ? result
                : default;
        }

        private static Vector4 CalculateOffsetScale(RectTransform rt, Vector2 localPoint, Vector2 scale)
        {
            var rect = rt.rect;
