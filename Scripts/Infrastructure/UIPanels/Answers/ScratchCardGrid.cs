...
namespace Infrastructure.UIPanels.Answers
{
   public readonly struct ScratchCardGrid
    {
        public static ScratchCardGrid Generate(Bounds bounds, Vector2 brushScale, int gridScale, ScratchCardGrid previous)
        {
        ...
        public Vector2Int LocalPointToCell(Bounds bounds, Vector2 point)
        {
            var w = bounds.size.x / Width;
            var h = bounds.size.y / Height;

            var x = (point.x - bounds.size.x) / w;
            var y = (point.y - bounds.size.y ) / h;
